...
int main(int argc, char* argv[]) {
	...
	std::string filename_R = argv[1];
	std::string filename_G = argv[2];
	std::string filename_B = argv[3];

	mat<uint8_t> image_R;
	mat<uint8_t> image_G;
	mat<uint8_t> image_B;

	if (!read_pam(filename_R, image_R)) {
		return EXIT_FAILURE;
	}
	if (!read_pam(filename_G, image_G)) {
		return EXIT_FAILURE;
	}
	if (!read_pam(filename_B, image_B)) {
		return EXIT_FAILURE;
	}

	mat<uint8_t> image(image_R.rows(), image_R.cols(), 3);

	size_t i = 0;
	for (size_t r = 0; r < image.rows(); ++r) {
		for (size_t c = 0; c < image.cols(); ++c) {
			image[i] = image_R(r, c);
			image[i + 1] = image_G(r, c);
			image[i + 2] = image_B(r, c);
			i = i + 3;
		}
	}

	if (!write_pam("image.pam", image)) {
		return EXIT_FAILURE;
	}
